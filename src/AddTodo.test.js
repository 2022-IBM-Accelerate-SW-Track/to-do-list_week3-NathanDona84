...
afterEach(() => {
 ...
 test('test that App component can be deleted thru checkbox', () => {
  ...
  let a = true;
  try{
    const checkDel = screen.getByText(/Project 3/i);
  }
  catch(err){
    a = false;
  }
 ...
 test('test that App component renders different colors for past due events', () => {
  ...
  fireEvent.click(button);
  const in1 = screen.getByTestId(/Project 3/i).style.background;
  const in2 = screen.getByTestId(/Project 2/i).style.background;
  let a = in1 === in2;
  expect(a).toBe(false);
 });
